...
Figure 6 visualizes marker-gene expression directly on the BRAINUMAP map.

Biological idea:
- Use selected genes as biological probes to annotate regions with lineage, state, or tumor-associated signals.

Conceptual flow:
1. Keep one shared coordinate system.
2. Overlay one gene at a time using continuous color scales.
3. Compare expression hotspots across disease-defined regions.

Interpretation focus:
- Localized high-expression zones can indicate cell-state identity or pathway activation niches.
- Diffuse expression suggests broader, non-specific programs.

```{r setup}
# Biologist note:
# This setup block loads UMAP metadata and expression matrix,
# then prepares shared color scales/themes for gene-expression overlays.
...
```{r}
# Biologist note:
# Figure 6: for each mismatch-repair gene, project expression intensity onto UMAP.
# This shows where pathway member genes are most active across samples.
...
````{r}
# Biologist note:
# Supplemental Figure 6 repeats the same expression-overlay strategy
# for genes in the BIOCARTA RELA pathway.
