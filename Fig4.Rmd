...
Figure 4 focuses on cross-disease and cross-age comparisons in the shared transcriptomic space.

Biological idea:
- Compare pediatric and adult tumor contexts to identify overlap, divergence, and potential lineage relationships.

Conceptual steps:
1. Define disease group color dictionaries.
2. Render disease-specific overlays on common coordinates.
3. Compare where groups co-localize or separate.

Interpretation focus:
- Co-localization can indicate shared programs.
- Separation can indicate distinct developmental or oncogenic states.


```{r}
# Biologist note:
# This setup chunk defines pediatric/adult color dictionaries and plotting theme,
# then prepares the workspace used for Figure 4 disease-level visualizations.
rm(list=ls())
...
```{r}
# Biologist note:
# Figure 4A: map only CBTN samples and color by pediatric diagnosis class.
# only CBTN 
...
```{r}
# Biologist note:
# Figure 4B: build the full BRAIN-UMAP disease map by combining
# adult subtype relabeling with pediatric diagnosis relabeling.
...
```{r}
# Biologist note:
# Supplemental Figure 4 shows finer pediatric subtype structure,
# specifically ependymoma subgroups and medulloblastoma molecular subtypes.
umap_data1 = read.delim("tables/umapdata_with_CBTN_8_27_2021_with_metadata.txt", header=T, stringsAsFactors = FALSE)
library(readxl)
...
```{r}
# Biologist note:
# This supplemental chunk focuses on GTEx+CBTN view,
# with one panel for broad groups and one panel for pediatric diagnosis detail.
