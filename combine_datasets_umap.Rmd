...
This notebook creates the combined reference space used throughout the project.

What the code is doing:
1. Load adult reference expression and pediatric CBTN expression.
2. Restrict both matrices to the same shared genes.
3. Merge metadata and expression into one joint matrix.
4. Use ComBat to reduce technical batch effects while preserving biological structure.
5. Run dimensionality reduction (PCA, t-SNE, UMAP) to produce a disease landscape.

How to interpret results:
- Nearby points in UMAP/t-SNE are transcriptionally similar.
- Separation can reflect tumor biology, developmental context, or technical effects.
- Batch correction should reduce platform/study-driven separation while keeping known biological groups coherent.

```{r setup}
# Biologist note:
# This setup block loads the adult-brain expression matrix and the CBTN matrix,
# keeps only genes shared between datasets, then creates one combined matrix.
# It also applies ComBat to reduce technical batch effects before visualization.
rm(list=ls())
...
```{r}
# Biologist note:
# Here we build the baseline map WITHOUT batch correction.
# We calculate UMAP (2D and 3D), PCA, and tSNE, then save coordinates for plotting.
...
```{r}
# Biologist note:
# This is the main BRAIN-UMAP build WITH ComBat batch correction.
# Output table from this chunk is the central coordinate file used by most figure scripts.
...
```{r}
# Biologist note:
# This chunk creates a subset map for normal brain controls (GTEx) and pediatric tumors (CBTN).
# It is used to study tumor-to-normal proximity in reduced-dimensional space.
...
```{r}
# Biologist note:
# This chunk creates a subset map for adult glioma cohorts (TCGA-GBM/TCGA-LGG) plus CBTN.
# It supports direct pediatric-versus-adult tumor comparisons.
...
```{r}
# Biologist note:
# This creates a lookup table linking RNA-seq biospecimen IDs to WGS biospecimen IDs.
# It is needed when integrating transcriptomic and genomic (mutation/fusion) analyses.
...
```{r}
# Biologist note:
# This final step adds selected metadata columns to subset UMAP tables.
# It includes sanity checks to ensure sample ordering and coordinates stayed consistent.
rm(list=ls())
