...
Figure 7 overlays genomic alteration burden on transcriptomic neighborhoods.

Biological idea:
- Integrate mutation, copy-number, and fusion burden with expression-state structure to identify genotype-phenotype relationships.

Conceptual flow:
1. Harmonize alteration datasets to sample IDs used in UMAP.
2. Aggregate burden metrics or event indicators.
3. Project genomic features on the same embedding used for expression analyses.

Interpretation focus:
- Region-specific burden enrichment may indicate subtype-defining genomic drivers.
- Overlap or divergence from expression-defined clusters can reveal convergent vs distinct biology.
...
```{r setup}
# Biologist note:
# This setup chunk prepares mutation, fusion, copy-number, and expression inputs,
# then computes burden-style overlays for Figure 7.
rm(list=ls())
...
```{r}
# Biologist note:
# Figure 7 combines disease-map context with genomic burden overlays
# (mutation count, fusion count, and copy-number gain/loss burden).
...
```{r}
# Biologist note:
# Supplemental Figure 7 summarizes per-sample mutation and fusion burdens
# as boxplots stratified by tumor type.
