...
Figure 1 introduces the global transcriptional map and key cohort labels.

Biological idea:
- Build intuition for how normal brain tissue, adult gliomas, and pediatric tumors occupy the shared expression landscape.

What this notebook emphasizes:
1. Consistent visual encoding (color, size, labeling) across panels.
2. Metadata-driven overlays to show diagnosis and tissue context.
3. High-level structure before focusing on subtype-specific analyses in later figures.

Interpretation focus:
- Broad clustering patterns indicate major biological programs and disease-state differences.


```{r setup}
# Biologist note:
# This setup block defines common plot style, colors, and font sizes,
# then loads the metadata-annotated UMAP table used for Figure 1 panels.
...
```{r}
# Biologist note:
# Figure 1A: color all samples by cohort/source dataset to show global structure of BRAIN-UMAP.
...
```{r}
# Biologist note:
# Figure 1B: focus on GTEx samples and color by brain tissue region.
# This reveals whether normal anatomical regions cluster separately.
...
```{r}
# Biologist note:
# Supplemental Figure 1A compares embeddings with and without batch correction
# using UMAP, PCA, and tSNE side-by-side.
...
```{r}
# Biologist note:
# Supplemental Figure 1B overlays GTEx sample-level covariates
# (PMI, age, RNA quality, sex, Hardy score, preparation) on UMAP.
