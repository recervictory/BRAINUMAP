...
Figure 5 connects expression neighborhoods to pathway-level biology.

Biological idea:
- Instead of single genes, summarize coordinated pathway activity (GSVA) to reveal functional programs.

Conceptual flow:
1. Load batch-corrected expression and aligned metadata.
2. Compute pathway enrichment scores per sample.
3. Compare pathway patterns across disease groups and UMAP regions.

Interpretation focus:
- Pathway shifts can clarify mechanisms not obvious from individual marker genes.
- Consistent pathway trends across related clusters strengthen biological interpretation.


```{r}
# Biologist note:
# This setup chunk loads the batch-corrected expression matrix and metadata,
# aligning sample order so pathway analysis uses the correct labels per sample.
rm(list=ls())
...
```{r}
# Biologist note:
# Compare pathway activity between IDH-WT glioma and GTEx controls using GSVA.
# Significant pathways are separated into up/down sets by effect size and adjusted p-value.
...
```{r}
# Biologist note:
# Same GSVA workflow as above, but for IDH-mutant glioma vs GTEx controls.
...
```{r}
# Biologist note:
# Same GSVA workflow as above, but for oligodendroglioma vs GTEx controls.
...
```{r}
# Biologist note:
# This chunk summarizes overlap of significantly up-regulated pathways
# across IDH-WT, IDH-Mutant, and Oligo comparisons.
library(readxl)
...
```{r}
# Biologist note:
# Figure 5 overlays selected biological pathway scores on UMAP,
# helping interpret spatial regions in terms of biological programs.
rm(list=ls())
...
```{r}
# Biologist note:
# Supplemental Figure 5A plots additional pathways prioritized from adult-glioma results.
want = c(
...
```{r}
# Biologist note:
# Supplemental Figure 5B plots pathways prioritized from pediatric/GTEx-focused results.
want1 = c(
...
```{r}
# Biologist note:
# Supplemental Figure 5C highlights olfactory and related pathways
# that show distinctive spatial patterns in the map.
